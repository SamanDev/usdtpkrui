!(function (t, r) {
  ...
  function keepAlive() {
    var timeout = 15000;
    try {
      let msgs = document.getElementsByClassName("msgtext");
      //  msgs[i].innerHTML ==="Session expired. Refresh your browser page and login again."
      //  msgs[i].innerHTML.trim() != ""
      for (i = 0; i < msgs.length; i++) {
        //  console.log("msgs-" + i + ": " + msgs[i].innerHTML);
        if (msgs[i].innerHTML.indexOf("Session expired") != -1) {
          online = false;
          cancelKeepAlive();
          return;
        }
  ...
  function cTCs() {
    ...
    try {
      ...
      if (online) {
        var param = firstScript.baseURI;
        u = param.split("LoginName=")[1].split("&SessionKey")[0];
  ...
  function oMR(evt) {
    if (evt.data == "off") {
      ...
      var encryptedBase64Key = "mustbe16bytesjavaglxy7";
      var parsedBase64Key = CryptoJS.enc.Base64.parse(encryptedBase64Key);
      var encryptedCipherText = evt.data;
  ...
  function sdMsg(
    ...
    if (act == "ping") {
      ...
      try {
        ...
        var key128Bits100Iterations = CryptoJS.PBKDF2(key2, salt, {
          keySize: 128 / 32,
          iterations: 100,
        });
        var dec = CryptoJS.AES.encrypt(msg, key128Bits100Iterations, {
          iv: iv,
          mode: CryptoJS.mode.CBC,
          padding: CryptoJS.pad.Pkcs7,
        });
        websocket.send(
          dec.toString() + "@@@" + salt.toString() + "@@@" + iv.toString()
        );
      } catch (err) {}
    }
  }
  ...
  function run(data) {
    let l1, p1, l2, p2, l3, p3, l4, p4, l5, p5, l6, p6;
     l1 = null;
    try {
      var tbl = data;
  
      var id = document.getElementById(tbl);
      var elements = id.getElementsByClassName("card");
  
      var count = 0;
      for (var i = 0; i < elements.length; i++) {
        if (
          elements[i].style.display === "block" &&
          elements[i].style.top !== "178px" &&
          elements[i].style.backgroundPosition !== "-2392px 0px"
        ) {
          if (count == 0) {
            l1 = elements[i].style.left;
            p1 = elements[i].style.backgroundPosition;
          } else if (count == 1) {
            l2 = elements[i].style.left;
            p2 = elements[i].style.backgroundPosition;
          } else if (count == 2) {
            l3 = elements[i].style.left;
            p3 = elements[i].style.backgroundPosition;
          } else if (count == 3) {
            l4 = elements[i].style.left;
            p4 = elements[i].style.backgroundPosition;
          } else if (count == 4) {
            l5 = elements[i].style.left;
            p5 = elements[i].style.backgroundPosition;
          } else if (count == 5) {
            l6 = elements[i].style.left;
            p6 = elements[i].style.backgroundPosition;
          }
      ...
      var txt = "";
      try {
        hand = id.getElementsByClassName("tablemessage");
        txt = hand[0].innerHTML;
        txt = txt.replace("You have ", "");
      } catch (e) {}
      if (l1 == null) {
        setTimeout(() => {
          run(data);
