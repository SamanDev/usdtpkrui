...
const Reward = (prop) => {
    ...
    try {
        var _lvl = prop.item.text.split(" ")[1].replace("Bonus", "").replace("Gift", "");
    } catch (error) {
        var _lvl = "";
    }
    if (_mode == "rakeback") {
        _txt = Trans("pokerrakeback");
    }
    if (_mode == "commission") {
        _txt = Trans("commitions");
    }
    if (_mode == "levels") {
        _txt = Trans("levelsreward");
    }
    if (_mode == "gpass") {
    ...
    return (
        <Grid verticalAlign="middle" divided="vertically" inverted padded="vertically">
            <Grid.Row
                className={loginToken?.username == prop.item.username && !prop.color ? "rewardred" : ""}
                onClick={() => {
            ...
            >
                <Grid.Column width={6}>
                    <div style={{ marginLeft: 10 }}>
                        <LevelIcon level={_lvl} number={_lvl} mode={_mode.toLowerCase()} text={prop.item.username} classinside={levelClassInside(_lvl - 1)} width="36px" />
                    </div>
                </Grid.Column>
                <Grid.Column width={10} textAlign="right" style={{ paddingRight: 20 }}>
                <ShowAmount amount={prop.item.amount} />
                    <small className="farsi rewardtext">{_txt}</small>
                    <small>{convertDateToJalali(prop.item.date)}</small>
                </Grid.Column>
                {(loginToken.username == "HangOver2" || (prop.item?.detail && prop.item.username == loginToken.username)) && (
                    <Grid.Column width={16} style={{ margin: 0 }}>
                        <CshList item={prop.item.detail} className={loginToken.username == "HangOver2" ? "hiddenmenu user" + prop.item.id : "user" + prop.item.id} />
                    </Grid.Column>
                )}
            </Grid.Row>
        </Grid>
